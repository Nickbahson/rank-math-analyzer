class Pluralize {
	...
	get( word ) {
		...
		var len = this.pluralizationRules.length

		// Iterate over the sanitization rules and use the first one to match.
		while ( len-- ) {
			var rule = this.pluralizationRules[len]
			if ( rule[0].test( word ) ) {
	...
	replace( word, rule ) {
		...
		return word.replace( rule[0], function( match, index ) {
			var result = self.interpolate( rule[1], arguments )
			if ( '' === match ) {
				return self.restoreCase( word[index - 1], result )
			}

			return self.restoreCase( match, result )
		})
	}
